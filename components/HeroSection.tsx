...
const TypingText = ({ text }: { text: string }) => {
	...
	return (
		<span className="inline-block whitespace-pre-line leading-relaxed">
			<span>
				I turn ideas into <span className="font-extrabold text-[#0D9488]">digital products</span> that matter —
			</span>
			<br />
			<span>
				{displayedSecondLine}
...
export default function HeroSection() {
	return (
		<section id="home" className="min-h-screen flex flex-col justify-center px-6 pt-24 pb-12 max-w-6xl mx-auto relative overflow-hidden md:overflow-visible">
			<div className="grid md:grid-cols-2 gap-12 md:gap-8 items-center w-full z-10 mt-10 md:mt-0">
				<motion.div
					initial="hidden"
					animate="visible"
				...
				>
					...
					<motion.h1
						...
						className="font-extrabold text-5xl md:text-6xl lg:text-[72px] leading-[1.05] mb-6 tracking-tight text-[var(--text-primary)]"
					>
					...
					<motion.div
						variants={{ hidden: { opacity: 0, y: 24 }, visible: { opacity: 1, y: 0, transition: { duration: 0.6 } } }}
						className="text-lg md:text-xl text-[var(--text-secondary)] max-w-lg leading-relaxed mb-10 font-medium"
					>
					...
					>
						<a
							href="#projects"
							className="bg-[var(--text-primary)] hover:bg-black text-white px-6 py-3.5 rounded-full font-bold transition-colors flex items-center gap-2 border border-[#18181B] shadow-md"
						>
						...
						<a
							...
							target="_blank"
							rel="noreferrer"
							className="bg-white border border-gray-200 hover:bg-gray-50 px-6 py-3.5 rounded-full font-bold transition-colors flex items-center gap-2 shadow-sm text-[var(--text-primary)]"
						>
				...
				<motion.div
					...
					className="relative w-full max-w-[260px] sm:max-w-sm md:max-w-md mx-auto md:ml-auto order-1 md:order-2"
				>
					<div className="absolute -right-8 top-16 h-56 w-56 rounded-full bg-[radial-gradient(circle_at_center,rgba(13,148,136,0.22),rgba(59,130,246,0.10)_42%,transparent_72%)] blur-3xl opacity-70"></div>
					<div className="absolute right-4 bottom-2 h-32 w-32 rounded-full bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.40),rgba(13,148,136,0.08)_45%,transparent_75%)] blur-2xl opacity-80"></div>
					<div className="relative aspect-square md:aspect-square w-full rounded-[36%_64%_58%_42%/42%_45%_55%_58%] p-3 sm:p-4 bg-white/35 backdrop-blur-2xl shadow-[0_18px_60px_rgba(15,23,42,0.08)] rotate-[-2deg]">
						...
						<div className="absolute -top-4 left-8 z-20 inline-flex items-center gap-2 rounded-full bg-white/95 px-4 py-2 text-sm font-extrabold text-[var(--text-primary)] shadow-[0_10px_24px_rgba(15,23,42,0.08)] border border-white/70 rotate-[3deg]">
