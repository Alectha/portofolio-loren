				...
				{projects.map((project, index) => (
					<FadeIn key={project.id} delay={index * 0.1} className={index === 0 ? "md:col-span-2" : ""}>
						<GlassCard
							...
							className="p-5 sm:p-6 md:p-8 h-full flex flex-col group"
						>
							<div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 mb-5 sm:mb-6">
								<div className={`px-4 py-1.5 rounded-full text-sm font-extrabold bg-white border border-[#E2E8F0] shadow-sm ${getTextColorClass(project.color)}`}>
									{project.category}
								</div>
								<span className="text-xs font-bold text-[var(--text-tertiary)]">{project.period}</span>
							</div>

							<h3 className="font-extrabold text-xl sm:text-2xl md:text-3xl mb-3 text-[var(--text-primary)] group-hover:text-[var(--accent-teal)] transition-colors">
								{project.title}
							</h3>
							<p className="text-[var(--text-secondary)] text-sm font-medium leading-relaxed mb-5 sm:mb-6 flex-grow">
								{project.summary}
							</p>
							...
							<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-sm font-bold text-[var(--text-primary)] border-t border-gray-200 pt-4">
